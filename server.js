...
function analyzeWebsitePurpose(text, title, keywords) {
    ...
    for (const [purpose, keywords] of Object.entries(purposes)) {
        matches[purpose] = keywords.filter(keyword => contentLower.includes(keyword)).length;
    }

    const mainPurpose = Object.entries(matches)
...
app.post('/api/analyze', async (req, res) => {
    try {
        ...
        $('meta[name="keywords"]').each((i, elem) => {
            keywords.push(...$(elem).attr('content').split(',').map(k => k.trim()));
        });
        ...
        $('img').each((i, elem) => {
            ...
            if (src && src.startsWith('http') && 
                (!width || parseInt(width) > 100)) { // Filter out tiny images
                images.push({ src, alt });
            }
        });
        ...
        $('a[href^="mailto:"]').each((i, elem) => {
            contacts.push({ type: 'email', value: $(elem).attr('href').replace('mailto:', '') });
        });
        $('a[href^="tel:"]').each((i, elem) => {
            contacts.push({ type: 'phone', value: $(elem).attr('href').replace('tel:', '') });
        });
